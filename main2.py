        ...
        if self.solveState == 1:
            ...
            for item in self.singlesDirection:
                ...
                if self.map[newx][newy] == 2:
                    ...
                    for index, item in enumerate(self.unvisitedNodes):
                        if item.pos == (newx,newy):
                            idx = index
                            break
                    if self.unvisitedNodes[idx].distanceFromZero > current.distanceFromZero + 1:
                        del self.unvisitedNodes[idx]
                        self.steps += 1
                    elif self.unvisitedNodes[idx].distanceFromZero == current.distanceFromZero + 1:
                        continue
                ...
                if self.unvisitedNodes == []:
                    self.unvisitedNodes.append(Node((newx,newy), current.distanceFromZero+1, current.pos))
                else:
                    for index, item in enumerate(self.unvisitedNodes): #add in order from smallest to largest
                        ...
                        if distance > current.distanceFromZero + 1: #once you find something with a greater distance insert into the list
                            self.unvisitedNodes.insert(index, Node((newx, newy), current.distanceFromZero+1, current.pos))
                            break
        ...
        if self.solveState == 1:
            ...
            for direction in self.singlesDirection:
                ...
                if (newx, newy) == goal:
                    ...
                    g = self.cellDetails[f"{current[1]}:{current[2]}"].g + 1
                ...
                g = self.cellDetails[f"{current[1]}:{current[2]}"].g + 1
                #h = (len(self.map)-2 - newx) + (len(self.map[0])-1-newy)#manhattan distance
                h = math.sqrt((len(self.map)-2 - newx)**2 + (len(self.map[0])-1-newy)**2)#diagonal distance
                #h = (len(self.map)-2 - newx)
                f = g + h

                #if cell is the lowest f of all cells at that option then add to open list
                if f"{newx}:{newy}" not in self.cellDetails or self.cellDetails[f"{newx}:{newy}"].f > f:
